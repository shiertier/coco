...
핸드오프는 한 에이전트가 대화의 일부를 다른 에이전트에게 위임하도록 합니다. 이는 서로 다른 에이전트가 특정 영역에 특화되어 있을 때 유용합니다. 예를 들어 고객 지원 앱에서는 예약, 환불 또는 FAQ를 처리하는 에이전트를 둘 수 있습니다.
...
- `onHandoff` – 핸드오프가 발생할 때 호출되는 콜백. `RunContext`와 선택적으로 파싱된 입력을 받음
- `inputType` – 핸드오프에 필요한 입력 스키마
- `inputFilter` – 다음 에이전트로 전달되는 히스토리 필터
...
기본적으로 핸드오프는 전체 대화 히스토리를 받습니다. 다음 에이전트로 전달될 내용을 수정하려면 `inputFilter`를 제공하세요.
...
프롬프트에서 핸드오프를 언급하면 LLM이 더 안정적으로 응답합니다. SDK는 `RECOMMENDED_PROMPT_PREFIX`를 통해 권장 접두어를 제공합니다.
