...
3. OpenAI API 키를 설정합니다. 키가 없다면 [이 안내](https://platform.openai.com/docs/quickstart#create-and-export-an-api-key)를 따라 OpenAI API 키를 생성하세요.

   ```bash
   export OPENAI_API_KEY=sk-...
   ```

   또는 `setDefaultOpenAIKey('<api key>')`를 호출해 프로그래밍적으로 키를 설정하고,
...
이 호출은 최종 출력과 실행 중 수행된 모든 작업을 포함하는 결과를 반환합니다.
...
에이전트에 도구를 제공하여 정보를 조회하거나 작업을 수행하게 할 수 있습니다.
...
추가 에이전트를 비슷한 방식으로 정의하여 문제를 더 작은 부분으로 나누고, 에이전트가 현재 작업에 더 집중하도록 할 수 있습니다. 또한 에이전트에 모델을 정의하여 서로 다른 문제에 서로 다른 모델을 사용할 수 있습니다.
...
여러 에이전트를 오케스트레이션하려면 에이전트에 `handoffs`를 정의할 수 있습니다. 그러면 에이전트가 다음 에이전트로 대화를 넘길 수 있습니다. 이는 실행 과정에서 자동으로 발생합니다.
...
실행이 끝난 후 결과의 `finalAgent` 속성을 보면 어떤 에이전트가 최종 응답을 생성했는지 확인할 수 있습니다.

## 에이전트 오케스트레이션 실행

Runner는 개별 에이전트의 실행, 발생 가능한 핸드오프, 도구 실행을 처리합니다.
...
이제 하나의 전체 예제로 모두 합쳐 보겠습니다. 이를 `index.js` 파일에 넣고 실행하세요.
...
- [에이전트](/openai-agents-js/ko/guides/agents) 구성 알아보기
