...
交接让一个智能体将对话的一部分委派给另一个智能体。这在不同智能体各自专长于特定领域时很有用。例如在客服应用中，你可以设置分别处理预订、退款或常见问题的智能体。

交接在 LLM 中以工具的形式呈现。如果你将对话交接给名为 `Refund Agent` 的智能体，工具名将是 `transfer_to_refund_agent`。
...
有时你希望 LLM 在调用交接时提供数据。定义一个输入 schema，并在 `handoff()` 中使用它。
...
当你的提示词提到交接时，LLM 的响应会更可靠。SDK 通过 `RECOMMENDED_PROMPT_PREFIX` 提供了一个推荐的前缀。
