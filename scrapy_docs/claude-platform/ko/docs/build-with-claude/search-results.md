...
소스 속성을 포함한 검색 결과를 제공하여 RAG 애플리케이션에 자연스러운 인용을 활성화합니다

---

검색 결과 콘텐츠 블록은 적절한 소스 속성을 포함한 자연스러운 인용을 활성화하여 웹 검색 품질의 인용을 사용자 정의 애플리케이션에 제공합니다. 이 기능은 Claude가 소스를 정확하게 인용해야 하는 RAG(검색 증강 생성) 애플리케이션에 특히 강력합니다.
...
- Claude Haiku 3.5 ([deprecated](/docs/ko/about-claude/model-deprecations)) (`claude-3-5-haiku-20241022`)

## 주요 이점

- **자연스러운 인용** - 모든 콘텐츠에 대해 웹 검색과 동일한 인용 품질 달성
- **유연한 통합** - 동적 RAG를 위한 도구 반환 또는 사전 가져온 데이터를 위한 최상위 콘텐츠로 사용
- **적절한 소스 속성** - 각 결과에는 명확한 속성을 위한 소스 및 제목 정보 포함
- **문서 해결 방법 불필요** - 문서 기반 해결 방법의 필요성 제거
- **일관된 인용 형식** - Claude의 웹 검색 기능의 인용 품질 및 형식과 일치

## 작동 방식

검색 결과는 두 가지 방식으로 제공할 수 있습니다:

1. **도구 호출에서** - 사용자 정의 도구가 검색 결과를 반환하여 동적 RAG 애플리케이션 활성화
2. **최상위 콘텐츠로** - 사전 가져온 또는 캐시된 콘텐츠를 위해 사용자 메시지에 직접 검색 결과 제공

두 경우 모두 Claude는 적절한 소스 속성을 포함하여 검색 결과의 정보를 자동으로 인용할 수 있습니다.
        ...
        content: [
          {
            ...
            tool_use_id: response.content[0].id,
            content: toolResult  // 검색 결과가 여기에 들어갑니다
          }
        ]
      }
    ]
...
사용자 메시지에 직접 검색 결과를 제공할 수도 있습니다. 이는 다음의 경우에 유용합니다:
- 검색 인프라에서 사전 가져온 콘텐츠
- 이전 쿼리의 캐시된 검색 결과
- 외부 검색 서비스의 콘텐츠
- 테스트 및 개발
...
검색 결과가 어떻게 제공되든 Claude는 자동으로 검색 결과의 정보를 사용할 때 인용을 포함합니다:
...
| `end_block_index` | integer | 콘텐츠 배열의 끝 위치 |

참고: `search_result_index`는 검색 결과가 어떻게 제공되었는지(도구 호출 또는 최상위 콘텐츠)에 관계없이 검색 결과 콘텐츠 블록의 인덱스(0 기반)를 나타냅니다.
...
# 그러면 더 많은 검색 결과와 함께 도구 결과를 제공합니다
```
...
`citations.enabled`가 `true`로 설정되면 Claude는 검색 결과의 정보를 사용할 때 인용 참조를 포함합니다. 이를 통해 다음이 가능합니다:
- 사용자 정의 RAG 애플리케이션에 대한 자연스러운 인용
- 독점 지식 기반과 인터페이스할 때 소스 속성
...
- 검색 결과 콘텐츠 블록은 Claude API, Amazon Bedrock 및 Google Cloud의 Vertex AI에서 사용 가능합니다
- 검색 결과 내에서는 텍스트 콘텐츠만 지원됩니다(이미지 또는 기타 미디어 없음)
- `content` 배열은 최소 하나의 텍스트 블록을 포함해야 합니다