...
Claude가 API 대화 내에서 데이터를 분석하고, 시각화를 생성하고, 복잡한 계산을 수행하고, 시스템 명령을 실행하고, 파일을 생성 및 편집하고, 업로드된 파일을 직접 처리할 수 있습니다.

---

Claude는 API 대화 내에서 데이터를 분석하고, 시각화를 생성하고, 복잡한 계산을 수행하고, 시스템 명령을 실행하고, 파일을 생성 및 편집하고, 업로드된 파일을 직접 처리할 수 있습니다.
코드 실행 도구를 사용하면 Claude가 보안 샌드박스 환경에서 Bash 명령을 실행하고 파일을 조작할 수 있습니다(코드 작성 포함).

<Note>
코드 실행 도구는 현재 공개 베타 버전입니다.

이 기능을 사용하려면 API 요청에 `"code-execution-2025-08-25"` [베타 헤더](/docs/ko/api/beta-headers)를 추가하세요.
</Note>
...
| Claude Sonnet 3.7 (`claude-3-7-sonnet-20250219`) ([지원 중단됨](/docs/ko/about-claude/model-deprecations)) | `code_execution_20250825` |
| Claude Haiku 4.5 (`claude-haiku-4-5-20251001`) | `code_execution_20250825` |
| Claude Haiku 3.5 (`claude-3-5-haiku-latest`) ([지원 중단됨](/docs/ko/about-claude/model-deprecations)) | `code_execution_20250825` |

<Note>
현재 버전 `code_execution_20250825`는 Bash 명령 및 파일 작업을 지원합니다. 레거시 버전 `code_execution_20250522`(Python만 해당)도 사용 가능합니다. 마이그레이션 세부 정보는 [최신 도구 버전으로 업그레이드](#upgrade-to-latest-tool-version)를 참조하세요.
</Note>
...
Claude에게 계산을 수행하도록 요청하는 간단한 예제입니다:
...
Claude는 샌드박스의 파일 조작 기능을 사용하여 파일을 직접 생성, 보기 및 편집할 수 있습니다:
...
<Note>
Files API와 함께 Code Execution을 사용하려면 두 개의 베타 헤더가 필요합니다: `"anthropic-beta": "code-execution-2025-08-25,files-api-2025-04-14"`
</Note>
...
1. **Files API를 사용하여 파일 업로드** ([Files API](/docs/ko/build-with-claude/files))
...
async function main() {
  ...
  for (const fileId of fileIds) {
    ...
    // ReadableStream을 Buffer로 변환하고 저장합니다
...
# 1. bash를 사용하여 파일 크기를 확인하고 데이터를 미리 봅니다
# 2. text_editor를 사용하여 CSV를 분석하고 시각화를 생성하는 Python 코드를 작성합니다
# 3. bash를 사용하여 Python 코드를 실행합니다
# 4. text_editor를 사용하여 발견 사항이 포함된 README.md를 생성합니다
# 5. bash를 사용하여 파일을 보고서 디렉토리로 구성합니다
```
...
- `text_editor_code_execution`: 파일 보기, 생성 및 편집(코드 작성 포함)
...
- `stdout`: 성공적인 실행의 출력
- `stderr`: 실행이 실패하면 오류 메시지
- `return_code`: 성공하면 0, 실패하면 0이 아닌 값
...
| text_editor | `string_not_found` | `old_str`을 파일에서 찾을 수 없습니다 (str_replace의 경우) |
...
- **외부 연결**: 아웃바운드 네트워크 요청이 허용되지 않습니다
- **샌드박스 격리**: 호스트 시스템 및 다른 컨테이너로부터 완전히 격리됨
- **파일 액세스**: 작업 공간 디렉토리로만 제한됨
- **작업 공간 범위 지정**: [Files](/docs/ko/build-with-claude/files)와 마찬가지로 컨테이너는 API 키의 작업 공간으로 범위가 지정됩니다
...
async function main() {
  ...
  const response2 = await anthropic.beta.messages.create({
    container: containerId,  // 동일한 컨테이너를 재사용합니다
    model: "claude-sonnet-4-5",
    betas: ["code-execution-2025-08-25"],
    max_tokens: 4096,
    messages: [{
...
[Messages Batches API](/docs/ko/build-with-claude/batch-processing)에 코드 실행 도구를 포함할 수 있습니다. Messages Batches API를 통한 코드 실행 도구 호출은 일반 Messages API 요청과 동일하게 가격이 책정됩니다.
...
코드 실행 도구는 [프로그래밍 방식 도구 호출](/docs/ko/agents-and-tools/tool-use/programmatic-tool-calling)을 지원하며, 이를 통해 Claude가 실행 컨테이너 내에서 프로그래밍 방식으로 사용자 정의 도구를 호출하는 코드를 작성할 수 있습니다. 이를 통해 효율적인 다중 도구 워크플로우, Claude의 컨텍스트에 도달하기 전에 데이터 필터링, 복잡한 조건부 논리가 가능합니다.
...
[프로그래밍 방식 도구 호출 설명서](/docs/ko/agents-and-tools/tool-use/programmatic-tool-calling)에서 자세히 알아보세요.

## Agent Skills와 함께 코드 실행 사용

코드 실행 도구를 사용하면 Claude가 [Agent Skills](/docs/ko/agents-and-tools/agent-skills/overview)를 사용할 수 있습니다. Skills는 Claude의 기능을 확장하는 지침, 스크립트 및 리소스로 구성된 모듈식 기능입니다.

[Agent Skills 설명서](/docs/ko/agents-and-tools/agent-skills/overview) 및 [Agent Skills API 가이드](/docs/ko/build-with-claude/skills-guide)에서 자세히 알아보세요.