...
확장 사고를 언제 사용할지 결정하는 방법에 대해서는 [확장 사고 모델](/docs/ko/about-claude/models/extended-thinking-models)을 참조하세요.

## 시작하기 전에

이 가이드는 이미 확장 사고 모드를 사용하기로 결정했으며, [확장 사고 모델 시작하기](/docs/ko/about-claude/models/extended-thinking-models#getting-started-with-extended-thinking-models)에 대한 기본 단계와 [확장 사고 구현 가이드](/docs/ko/build-with-claude/extended-thinking)를 검토했다고 가정합니다.

### 확장 사고를 위한 기술적 고려사항

- 사고 토큰은 최소 1024 토큰의 예산을 가집니다. 최소 사고 예산으로 시작하여 필요와 작업 복잡성에 따라 점진적으로 증가시키는 것을 권장합니다.
- 최적의 사고 예산이 32K를 초과하는 워크로드의 경우, 네트워킹 문제를 피하기 위해 [배치 처리](/docs/ko/build-with-claude/batch-processing)를 사용하는 것을 권장합니다. 모델이 32K 토큰 이상으로 사고하도록 하는 요청은 시스템 타임아웃과 열린 연결 제한에 부딪힐 수 있는 장시간 실행 요청을 야기합니다.
- 확장 사고는 영어에서 최고의 성능을 발휘하지만, 최종 출력은 [Claude가 지원하는 모든 언어](/docs/ko/build-with-claude/multilingual-support)로 가능합니다.
- 최소 예산 미만의 사고가 필요한 경우, 사고를 끄고 XML 태그(`<thinking>`와 같은)를 사용한 전통적인 연쇄 사고 프롬프팅과 함께 표준 모드를 사용하는 것을 권장합니다. [연쇄 사고 프롬프팅](/docs/ko/build-with-claude/prompt-engineering/chain-of-thought)을 참조하세요.
...
[멀티샷 프롬프팅](/docs/ko/build-with-claude/prompt-engineering/multishot-prompting)은 확장 사고와 잘 작동합니다. Claude에게 문제를 어떻게 생각해야 하는지 예시를 제공하면, 확장 사고 블록 내에서 유사한 추론 패턴을 따를 것입니다.
...
확장 사고가 꺼져 있을 때는 표준 `assistant` 응답 텍스트 [미리 채우기](/docs/ko/build-with-claude/prompt-engineering/prefill-claudes-response)가 여전히 허용됩니다.
  ...
  <section title="제약 최적화 문제">
    ...
    <Tabs>
      <Tab title="표준 프롬프트">
        ...
        <TryInConsoleButton
          userPrompt="일본으로 일주일간의 휴가를 계획하세요."
          thinkingBudgetTokens={16000}
        >
          콘솔에서 시도하기
        </TryInConsoleButton>
...
<CardGroup>
  ...
  <Card title="확장 사고 가이드" icon="code" href="/docs/ko/build-with-claude/extended-thinking">
