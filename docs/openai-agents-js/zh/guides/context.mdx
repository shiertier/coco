...
“上下文”一词有多重含义。通常有两大类你可能关心的上下文：

1. 运行期间代码可访问的**本地上下文**：工具所需的依赖或数据、如 `onHandoff` 的回调，以及生命周期钩子。
...
当调用 LLM 时，它能看到的唯一数据来自对话历史。若要提供额外信息，你有以下选项：

1. 将其添加到智能体的 `instructions`（也称为 system 或 developer 消息）。它可以是静态字符串，或接收上下文并返回字符串的函数。
2. 在调用 `Runner.run()` 时将其包含在 `input` 中。此方式类似于 instructions，但允许你将消息放在[指令链](https://cdn.openai.com/spec/model-spec-2024-05-08.html#follow-the-chain-of-command)的较低位置。
3. 通过函数工具暴露出来，以便 LLM 按需获取数据。
