...
デフォルトでは、SDK は最初のインポート時に環境変数 `OPENAI_API_KEY` を読み取ります。変数を設定できない場合は、手動で `setDefaultOpenAIKey()` を呼び出せます。
...
独自の `OpenAI` クライアントインスタンスを渡すこともできます。指定しない場合は、デフォルトのキーを使って自動的に作成されます。
...
`@openai/agents` の `getLogger(namespace)` を使用して、独自モジュール向けの名前空間付きロガーを取得できます。
