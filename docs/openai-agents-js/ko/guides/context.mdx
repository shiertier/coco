...
컨텍스트는 과부하된 용어입니다. 신경 써야 할 컨텍스트는 크게 두 가지가 있습니다:
...
LLM이 호출될 때 볼 수 있는 데이터는 대화 기록에서만 옵니다. 추가 정보를 제공하려면 다음과 같은 옵션이 있습니다:

1. 에이전트 `instructions`에 추가합니다. 시스템 또는 개발자 메시지라고도 합니다. 컨텍스트를 받아 문자열을 반환하는 함수나 정적 문자열일 수 있습니다.
2. `Runner.run()`을 호출할 때 `input`에 포함합니다. instructions 기법과 비슷하지만, 메시지를 [지휘 체계](https://cdn.openai.com/spec/model-spec-2024-05-08.html#follow-the-chain-of-command)에서 더 낮은 위치에 둘 수 있습니다.
3. 함수 도구를 통해 노출하여 LLM이 필요 시 데이터를 가져올 수 있게 합니다.
4. 검색 또는 웹 검색 도구를 사용해 파일, 데이터베이스, 웹의 관련 데이터에 근거해 응답을 생성합니다.
