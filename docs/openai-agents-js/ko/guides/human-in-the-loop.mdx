...
1. 에이전트가 도구(하나 이상)를 호출하기로 결정하면 `needsApproval`을 평가하여 이 도구에 승인이 필요한지 확인합니다.
2. 승인이 필요한 경우, 에이전트는 승인이 이미 승인되었거나 거부되었는지 확인합니다.
   - 승인이 승인 또는 거부되지 않은 경우, 도구 호출을 실행할 수 없다는 정적 메시지를 에이전트에 반환합니다.
   - 승인/거부가 없는 경우 도구 승인 요청이 트리거됩니다.
3. 에이전트는 모든 도구 승인 요청을 수집하고 실행을 인터럽트합니다.
4. 인터럽션(중단 처리)이 있는 경우 [실행 결과](/openai-agents-js/ko/guides/results)에 보류 중인 단계를 설명하는 `interruptions` 배열이 포함됩니다. 도구 호출에 확인이 필요한 경우 `type: "tool_approval_item"`을 가진 `ToolApprovalItem`이 나타납니다.
5. 도구 호출을 승인 또는 거부하려면 `result.state.approve(interruption)` 또는 `result.state.reject(interruption)`을 호출할 수 있습니다.
6. 모든 인터럽션을 처리한 후, 원래 전체 실행을 트리거한 `agent`를 `runner.run(agent, state)`에 `result.state`와 함께 전달하여 실행을 재개할 수 있습니다.
...
휴먼인더루프 (HITL) 플로우는 서버를 계속 실행하지 않고도 오랜 시간 동안 인터럽트될 수 있도록 설계되었습니다. 요청을 중지했다가 나중에 계속해야 하는 경우 상태를 직렬화하여 재개할 수 있습니다.
...
<Aside>
  이는 주로 에이전트에 변경을 가하는 동안 직렬화된 상태를 오랫동안 저장하려는
  경우에 적용됩니다.
</Aside>

승인 요청에 시간이 오래 걸리고 에이전트 정의를 의미 있게 버전 관리하거나 Agents SDK 버전을 올릴 계획이라면, 패키지 별칭을 사용해 두 버전의 Agents SDK를 병렬로 설치하여 자체 분기 로직을 구현하는 것을 권장합니다.

실제로는 자체 코드에 버전 번호를 부여하고 이를 직렬화된 상태와 함께 저장한 다음, 역직렬화를 코드의 올바른 버전으로 유도하는 방식입니다.
