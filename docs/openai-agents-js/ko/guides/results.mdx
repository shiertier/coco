...
- `unknown` — 출력 타입으로 JSON 스키마가 정의된 경우. 이때 JSON은 파싱되지만 타입은 수동으로 검증해야 합니다.
- `z.infer<outputType>` — 출력 타입으로 Zod 스키마가 정의된 경우. 출력은 자동으로 해당 스키마에 따라 파싱됩니다.
...
- `result.output` — 전체 에이전트 실행의 출력 포함

채팅과 유사한 사용 사례에서 `history`는 전체 히스토리를 유지하는 편리한 방법입니다:

<Code lang="typescript" code={historyLoop} title="히스토리 루프" />

## 마지막 에이전트

`lastAgent` 속성에는 마지막으로 실행된 에이전트가 포함됩니다. 애플리케이션에 따라, 이는 다음에 사용자 입력이 있을 때 유용한 경우가 많습니다. 예를 들어 프런트라인 분류 에이전트가 특정 언어 에이전트로 핸드오프하는 경우, 마지막 에이전트를 저장해 두었다가 사용자가 다시 메시지를 보낼 때 재사용할 수 있습니다.

스트리밍 모드에서는 현재 실행 중인 에이전트에 매핑되는 `currentAgent` 속성에 접근하는 것도 유용할 수 있습니다.

## 새 항목

`newItems` 속성에는 실행 중에 생성된 새 항목이 포함됩니다. 항목은 [`RunItem`](/openai-agents-js/openai/agents/type-aliases/runitem)입니다. 실행 항목은 LLM이 생성한 원문 항목을 래핑합니다. 이를 사용하면 LLM의 출력 외에도 해당 이벤트가 어떤 에이전트와 연관되어 있는지에 접근할 수 있습니다.
...
- [`RunToolApprovalItem`](/openai-agents-js/openai/agents/classes/runtoolapprovalitem): LLM이 도구 호출 승인을 요청했음을 나타냅니다. 원문 항목은 LLM의 도구 호출 항목입니다.

## 상태

`state` 속성에는 실행의 상태가 포함됩니다. 대부분의 `result`에 첨부된 정보는 `state`에서 파생되지만, `state`는 직렬화/역직렬화가 가능하며 [오류로부터 복구](/openai-agents-js/ko/guides/running-agents#exceptions)가 필요하거나 [`interruption`](#interruptions)을 처리해야 하는 경우 이후의 `run` 호출 입력으로도 사용할 수 있습니다.

## 인터럽션(중단 처리)

에이전트에서 `needsApproval`을 사용하는 경우, 계속 진행하기 전에 처리해야 하는 `interruptions`가 트리거될 수 있습니다. 이때 `interruptions`는 인터럽션을 유발한 `ToolApprovalItem`의 배열이 됩니다. 인터럽션을 처리하는 방법에 대한 자세한 내용은 [휴먼 인 더 루프 (HITL)](/openai-agents-js/ko/guides/human-in-the-loop) 가이드를 확인하세요.
...
`inputGuardrailResults`와 `outputGuardrailResults` 속성에는 가드레일 결과(있는 경우)가 포함됩니다. 가드레일 결과에는 로깅하거나 저장하고 싶은 유용한 정보가 포함될 수 있어, 이를 제공해 드립니다.

### 원본 입력

`input` 속성에는 run 메서드에 제공한 원본 입력이 포함됩니다. 대부분의 경우 필요하지 않지만, 필요할 때 사용할 수 있도록 제공됩니다.
