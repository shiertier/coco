...
The "clever" (but not too clever) part of `TestModel` is that it will attempt to generate valid structured data for [function tools](https://ai.pydantic.dev/tools/index.md) and [output types](https://ai.pydantic.dev/output/#structured-output) based on the schema of the registered tools.
...
[Learn about Gateway](https://ai.pydantic.dev/gateway) weather_app.py
