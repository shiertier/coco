...
`DialogTrigger` works with any focusable React Aria component (e.g. [Button](Button.md), [Link](Link.md), etc.). Use the `<Focusable>` component to wrap a custom trigger element such as a third party component or DOM element.
...
| `aria-current` | `boolean | "step" | "time" | "true" | "false" | "page" | "location" | "date" | undefined` | — | Indicates the element that represents the current item within a container or set of related elements. |
| `aria-describedby` | `string | undefined` | — | Identifies the element (or elements) that describes the object. |
...
| `aria-haspopup` | `boolean | "grid" | "dialog" | "menu" | "true" | "false" | "listbox" | "tree" | undefined` | — | Indicates the availability and type of interactive popup element, such as menu or dialog, that can be triggered by an element. |
