...
- **Local Mode (Dev Tool):**
  ...
  - **关键差异:** **所见即所得** —— Daemon 直接操作 FS，体验最优化。
...
- **零构建依赖:** 计算层预编译为 Rust 二进制，避免 `node-gyp`/Python 依赖地狱。
...
- **处理:** Daemon 直连 Rust Engine 解析；**强制 Debounce + 批处理**，避免事件风暴。
...
采用 **Rust Workspace** + **Git Submodule** 混合结构：
...
│   └── /coco-local         # Local Daemon (SQLite + LanceDB + ort)
├── /private                (Closed Source)
│   └── /coco-server        # SaaS Service (PostgreSQL + OpenAI)
└── Cargo.toml
```
...
| 模式            | 目标场景    | 启动方式        | Engine 拓扑          | 存储（Meta+Vector）   | 默认绑定  | 分发优先级     |
| :-------------- | :---------- | :-------------- | :------------------- | :-------------------- | :-------- | :------------- |
| **Server Mode** | 团队/服务器 | Docker/系统服务 | 独立容器/服务        | PostgreSQL + pgvector | 127.0.0.1 | Docker Image   |
| **Local Mode**  | 个人/本机   | `coco start`    | Sidecar (Preferred) / FFI (Optional) | SQLite + LanceDB      | 127.0.0.1 | npm/本地二进制 |

### 2.3 🟢 Local Mode (Hub & Spoke)

```mermaid
graph TD
    Consumer[MCP Client / Web Agent / CLI] --"HTTP/MCP"--> Daemon

    subgraph "Host Machine"
        subgraph "CoCo Daemon (Node.js)"
            Daemon[API Gateway]
            Daemon --> ProjectMgr[Project Manager]
            ProjectMgr --> Registry[Project Registry (ID <-> Source)]
            Registry --> MetaDB[(SQLite: Metadata)]
            ProjectMgr --> SvcA[Service: Project A]
            ProjectMgr --> SvcB[Service: Project B]
            SvcA --"Compute"--> EngineAdapter[Engine Adapter]
            SvcA --"Storage"--> DBAdapter[LanceDB Adapter]
        end

        subgraph "CoCo Engine (Rust Sidecar)"
            RustEngine[UDS/Named Pipe Server]
            RustEngine --> Embedder[ONNX Runtime (Singleton)]
            RustEngine --> Parser[Tree-sitter]
        end

        VectorDB[(LanceDB: Vectors)]
    end

    EngineAdapter <==>|"UDS/Named Pipe + Arrow/Protobuf"| RustEngine
    DBAdapter ==> VectorDB
```

#### 2.3.1 Local Mode: Sidecar vs FFI

**问题:** FFI 带来分发与稳定性成本（Native Module 兼容性、崩溃影响主进程），Sidecar 则有 IPC 序列化开销。

**建议:** Local Mode 默认采用 **Sidecar**，FFI 作为可选“Pro 加速”：

- **稳定优先:** Rust 崩溃不影响 Node 主进程，支持自动重启与隔离。
- **分发友好:** 下载匹配的二进制即可运行，避免 glibc/musl/arch 编译兼容坑。
- **IPC 足够快:** 本机使用 UDS/Named Pipe，性能接近 FFI，远高于 TCP。

**FFI 适用场景:** 极低延迟、超高频调用、明确可控的运行环境（Pro/可选插件）。

**Server Mode:** 仍保持 Sidecar/Microservice。

### 2.4 🔵 Server Mode (First-Class Docker)

- **Daemon 与 Engine 独立部署**，通过 gRPC/Protobuf 或 Arrow IPC 通信，降低序列化开销。
- **Daemon 不负责 Spawn**，由容器编排系统管理进程生命周期。

```mermaid
graph TD
    Client[Apps / IDEs] --> Daemon
    ...
    Tester --"Batch Push (Verified Artifacts)"--> Daemon

    subgraph "CoCo Server (Retrieval Endpoint)"
        Daemon[CoCo Daemon]
        Engine[CoCo Engine]
        Daemon <==> Engine
        Daemon --> DB[(PostgreSQL/VectorDB)]
    end

    Consumer[App / Agent] --"Query"--> Daemon
    Daemon --"Final Results (Verified)"--> Consumer
```
...
- **向量分层:** `StorageBackend` 作为**业务语义端口**（如 `search_similar_docs`），具体实现分别在 Local/Server 中绑定：
  - Server Mode (Network): PostgreSQL + pgvector 或 Specialized VDB (Qdrant/Milvus)。**不支持嵌入式数据库**。
...
- **协议策略: Public JSON + Internal Binary:**
  ...
  - **Internal IPC:** Daemon <-> Engine 允许使用 Arrow IPC / Protobuf，避免 JSON 序列化开销。
...
- **Smart Lock Check:**
  - 检查 `~/.coco/daemon.lock`，读取 PID 并验证进程存活。
  - 不存在则清理锁文件并继续启动。
...
- **Windows 注意:** `process.kill` 语义可能不一致，建议使用 `tasklist` 或 `tree-kill` 检测逻辑。
...
- **Engine 管理:**
  - Local Mode：Daemon Spawn `coco-engine`。
  - Server Mode：Daemon 连接 `COCO_ENGINE_ADDR`，不管理 Engine 进程。
- **版本握手:**
...
- **零拷贝与无状态 (Daemon):**
  ...
  - Rust 返回的 JSON 或二进制流必须直接 pipe 给响应或写入存储，禁止进入 JS 对象。
  - 设计目标是长期运行的稳定内存占用，避免通过重启掩盖泄漏。
...
- **计算下沉:**
  ...
  - Embedding 与 AST 解析全部由 Rust Engine 执行。
  - 数据流: `Node.js Service` -> `Engine Adapter` -> `Rust Engine`.
  - **内部协议:** Daemon 与 Engine 之间优先使用 Arrow IPC / Protobuf，避免 JSON 在高频/大数据场景的开销。

- **控制/数据分离 (Control vs Data Plane):**
  - **Node.js 只做编排:** 鉴权、策略选择、任务编排与状态查询。
  - **Rust 只做处理:** 解析、向量化、检索与写入。
  - Node.js **不得解析海量数据**；大数据只允许“引用传递”。

- **引用传递 (Pass-by-Reference):**
  - 大体积内容先写入临时存储（本地文件/S3/对象存储）。
  - Node.js 仅传递 `blob_ref`/`url` 与 `plan` 到任务队列。
  - Rust Worker 直接拉取数据并写入最终存储，避免 V8 内存爆炸。
...
- **增量同步与批处理 (Ingestion Protocol):**
  ...
  - **Local Mode (Direct):** Daemon 直接扫描用户本地 FS。
...
- **资源池化:**
  ...
  - Rust Engine 内部维护全局单例的 Embedding 模型。
  - 多项目共享，不重复加载。

- **多客户端并发:**

  - Daemon 必须支持 IDE/CLI/脚本同时连接，保持单进程内资源复用。
  - Node.js 不直接反序列化大规模向量数据；批量索引应由 Engine 直接写入存储。
...
目的：让 Rust 可执行“可配置策略”，Node.js 负责生成计划并快速迭代业务逻辑。

- **计划输入:** `IndexingPlan` / `QueryPlan`（JSON/YAML），由 Node.js 生成并传给 Rust。
...
- Node.js 可为特定任务附带 `wasm_module_ref`。
- Rust Engine 使用 `wasmtime` 执行 WASM 规则，避免核心引擎硬编码。

#### 3.3.1 Server Mode: 双引擎拓扑 (Query vs Ingest)

目的：隔离查询低延迟与构建高吞吐，避免批量导入抢占查询资源。

| 特性 | **Query Engine (查询专用)** | **Ingest Engine (构建专用)** |
| :--- | :--- | :--- |
...
- **存储后端灵活度:**
  ...
  - **Server Mode (Network Only):**
    ...
    - **允许:** Specialized Vector DB (Qdrant, Milvus, Weaviate)。
...
- **流程:**
  1.  **Search:** 查库发现。
  2.  **Validation:** Daemon 校验 FS mtime。
  3.  **Hydration:** Daemon 读取 FS 覆盖旧内容 (Read-Only fix)。
...
请求示例（导入/批量写入时传入 `indexing_config`）：
...
```jsonc
{
  ...
  "items": [ ... ],
  "indexing_config": {
    "config_id": "strategy_big_chunks_bge_model",
    "chunking": {
      "strategy_name": "markdown_header",
      "chunk_size": 1024,
      "chunk_overlap": 200
    },
    "embedding": {
...
indexing_strategies:
  - config_id: "small_chunks_minilm"
    chunking: { strategy_name: "fixed_token", chunk_size: 256, chunk_overlap: 50 }
    embedding: { model_name: "all-MiniLM-L6-v2" }
  - config_id: "large_chunks_bge"
    chunking: { strategy_name: "fixed_token", chunk_size: 1024, chunk_overlap: 200 }
...
| 组件         | 角色      | 技术选型                                              | 关键说明                                                       |
| :----------- | :-------- | :---------------------------------------------------- | :------------------------------------------------------------- |
| **Daemon**   | 控制/业务 | Node.js (TypeScript)                                  | Fastify/Express, 负责 HTTP/Auth/Registry。                     |
| **Engine**   | 计算/推理 | Rust                                                  | Axum/Tonic, 集成 `ort` 与 `tree-sitter`。                      |
...
| **CLI**      | 命令行    | Node.js                                               | Local 启动器与 Engine 下载器。                                 |
...
> [!IMPORTANT] > **Server Engine 职责明确化:**
> 即使 Embedding 由 OpenAI API 提供，Rust Engine 在 Server Mode 下依然**不可或缺**：
>
> - **AST Parsing (Tree-sitter):** 将代码解析为语义结构。Node.js 无法高效处理 (CPU Bound)。
> - **Smart Chunking:** 将长文件智能切分为 512 Token 的语义块，保留上下文完整性。
> - **Pipeline:** Engine 执行 `Parse -> Chunk -> Call OpenAI -> Store`。它是**计算单元**，而非 API 转发器。
...
- **混合内容的关键优势（代码块嵌入）**
  ...
  - 这使 CoCo 能将“函数说明 + 示例代码 + 参数表格”作为一个完整语义块提取，避免 Node.js 生态中“多解析器拼装”的复杂与脆弱。
...
- **多语言语法膨胀与可扩展性（WASM 插件化为核心）**
  ...
  - **必须: Tree-sitter WASM 插件化**
    - Rust 通过 `wasmtime` 动态加载 `.wasm` Grammar，Engine 仅内置最小 Core（如 Markdown/Plain）。
    - 语法包存放在 `~/.coco/grammars/`，按文件类型按需加载（如检测到 `.vue` 时加载 `tree-sitter-vue.wasm`）。
    - 若本地缺失，CLI 自动从 Release 拉取并缓存（可配置镜像源）。
    - 新增语言无需发布新 Engine，仅发布/下载 Grammar 包。
...
- **Local Optional:**
  - **优先:** 常用平台的二进制直接打入 npm 包，按 `os/cpu` 拆分为独立包（如 `@coco/core-darwin-arm64`），通过 `optionalDependencies` 自动分发，确保安装成功率。
  - **补充:** 提供 `install.js` 作为少数平台的补救路径，检测 OS/Arch/Libc（glibc/musl）后从 Release 拉取匹配二进制。
  - **Grammar 包:** `tree-sitter` 语法以 WASM 形式按需下载到 `~/.coco/grammars/`，不随引擎二进制打包。
  - **Fallback:** 提供 `coco setup`，允许手动指定本地二进制路径或重试下载，不在 install 阶段硬失败。
  - `coco start` 仅用于本地单机场景。

### 4.2 命令行体验 (CLI DX - Local Mode)

- **TUI Dashboard:** `coco start` 默认开启 TUI 模式 (`ink` / `ratatui`)。
...
| 配置项              | 说明                                                    |
| :------------------ | :------------------------------------------------------ |
| `COCO_MODE`         | `server` / `local`                                      |
| `COCO_HOST`         | 监听地址，默认 `127.0.0.1`                              |
| `COCO_PORT`         | 默认 `3456`                                             |
| `COCO_DB_URL`       | 数据库连接（Server: `postgres://`, Local: `sqlite://`） |
| `COCO_LANCEDB_PATH` | Local Mode LanceDB 路径                                 |
| `COCO_ADMIN_KEY`    | 管理端 Master Key                                       |
| `COCO_API_KEY`      | 客户端 API Key                                          |
| `COCO_ENGINE_ADDR`  | Engine gRPC 地址（Server Mode 必填）                    |
...
### 5.1 Protocol Standard (JSON Only)
...
**风险:** Server Mode 中 Node.js Gateway 若解析大体积响应（100MB 级向量/文档），会导致 V8 堆暴涨、Event Loop 阻塞。

**原则:** Node.js 只做**认证与路由**，对大数据流保持**全链路透传**。
...
- `GET /v1/sys/health` : Daemon + Engine 状态与签名（含版本号）。
...
- `POST /v1/docs/import` : 触发导入任务（需 `project_id`，可携带 `indexing_config`）。
...
- `x-coco-project-id: <UUID>` (Docs 必填)
...
### Phase 2: Daemon Core (Node.js)

- [ ] 实现 `ProjectRegistry` (SeaORM: PostgreSQL / SQLite)。
- [ ] 实现 `EngineAdapter` (支持外部 Engine 地址)。
...
- [ ] 发布 Docker Images (`coco-daemon`, `coco-engine`)。
